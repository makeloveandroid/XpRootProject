...
class ModifyApplicationDexTask(val unZipFile: File, val appendDexFile: File, val applicationName: String) : Task<File, File>() {
...
//                    println("改造前:\n${classToSmali(classWrapper)}")
                    ...
                    val newMethod = InjectMethodBuilder.buildStaticContextMethod(staticMethod)
...
//                dexPool.internClass(classWrapper)
                ...
                // 将app的class 插入到追加的里面 防止65535问题
                appendApplication(classWrapper)
    ...
    private fun appendApplication(classWrapper: ClassDefWrapper) {
        val openDex = openDex(appendDexFile.absolutePath)
        val dexPool = DexPool(openDex.opcodes)
        for (classDef in openDex.classes) {
            dexPool.internClass(classDef)
        }
        // 追加宿主application
        dexPool.internClass(classWrapper)
        appendDexFile.delete()
        dexPool.writeTo(FileDataStore(appendDexFile))
    }
