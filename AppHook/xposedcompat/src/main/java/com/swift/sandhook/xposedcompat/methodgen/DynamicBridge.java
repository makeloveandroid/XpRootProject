...
import lu.die.xposedcompat.SlimWrapper;
...
public final class DynamicBridge {
    ...
    public static synchronized void hookMethod(Member hookMethod, XposedBridge.AdditionalHookInfo additionalHookInfo) {
        ...
        try {
            ...
            if (stub != null) {
                HookWrapper.HookEntity hookEntity = new HookWrapper.HookEntity(hookMethod,
                        stub.hook, stub.backup, false);
                if(HookMode.currentHookMode.equals(HookMode.HOOKMODE_SANDHOOK))
                    SandHook.hook(hookEntity);
                else if(HookMode.currentHookMode.equals(HookMode.HOOKMODE_SLIMHOOK))
                    SlimWrapper.hookMethod(hookEntity);
